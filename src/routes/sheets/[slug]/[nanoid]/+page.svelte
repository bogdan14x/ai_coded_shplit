...
<main class="max-w-2xl mx-auto px-4 py-6 min-h-screen bg-neutral-950 text-neutral-300">
  {#if data.sheet}
    <div class="mb-6">
      ...
      <!-- Participants Avatar Group and Copy Button inline -->
      <div class="flex items-center justify-between gap-4 mt-2">
        <!-- Left: Participants Avatar Group -->
        ...
        <!-- Right: Copy Invite Link Button -->
        <button
          onclick={copyToClipboard}
          class="share-button px-3 py-1.5 bg-neutral-900 hover:bg-neutral-800 text-neutral-400 text-sm rounded-lg border border-neutral-700 hover:border-neutral-600 transition-all duration-200 flex items-center gap-1.5 cursor-pointer"
        >
          {#if copySuccess}
          ...
          {:else}
            <svg class="w-3.5 h-3.5 text-[#CB8E4C]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
